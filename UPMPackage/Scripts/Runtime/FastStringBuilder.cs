...
namespace RTLTMPro {
  public class FastStringBuilder {
    ...
    public FastStringBuilder(int[] unicodeArray) : this(unicodeArray.Count()) {
      SetValue(unicodeArray);
    }

    public FastStringBuilder(List<int> unicodeList) : this(unicodeList.Count) {
      SetValue(unicodeList.ToArray());
    }
    ...
    public void SetValue(int[] unicodeArray) {
      length = unicodeArray.Length;
      EnsureCapacity(length, false);
      unicodeArray.CopyTo(array, 0);
    }
